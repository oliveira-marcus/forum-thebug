...
export default function Finances() {
  ...
  useEffect(() => {
    if (sortBy === "date") {
      postService.getAllPosts(1, 20, "Finances").then((responseData) => setFeedInfo(responseData));
    } else {
      feedService.getFeed(1, 20, "Finances").then((responseData) => setFeedInfo(responseData));
    }
  }, [sortBy]);
